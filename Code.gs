 ...
 *
 * IMPORTANT:
 * - Do not put spreadsheet IDs or admin passwords in index.html.
 * - Set SPREADSHEET_ID in Script Properties.
 * - On first run, run setup() once from the Apps Script editor.
 */
...
function submitForm_(b) {
  ...
  // HoTen and CCCD are taken from DSCNV/session, not trusted from browser.
  const existing = findLatestResponseByMaNV_(s.maNV);
  upsertResponse_(s, {
    Consent:'Đồng ý',
...
function adminChangePassword_(b) {
  ...
  sh.getRange(admin.row,2,1,5).setValues([[newHash,newSalt,admin.active,admin.createdAt,new Date()]]);
...
function findAdmin_(username) {
  ...
  for (let i=1;i<rows.length;i++) {
    if (String(rows[i][0]).trim() === username) {
      return {row:i+1, username, hash:String(rows[i][1]), salt:String(rows[i][2]), active:rows[i][3], createdAt:rows[i][4]};
    }
  }
  return null;
}
...
function upsertResponse_(session, patch, existing) {
  ...
  if (existing && existing._row) {
    ...
    // Locate the actual row to avoid creating duplicates.
...
function randomHex_(n) {
  return bytesToHex_(Utilities.getRandomBytes(n));
}
...
function headerMap_(headers) {
  ...
  // allow common Vietnamese variants
